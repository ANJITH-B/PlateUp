...
const CardBox: React.FC = () => {
    ...
    const maxDrag = sm ? 115 : 150;
    const { scrollYProgress } = useViewportScroll();

    const logoOpacity = useTransform(dragX, [0, maxDrag], [0.25, 1]);
    const logoShadow = useTransform(dragX, [0, maxDrag], [0.0, 1]);
    ...
    return (
        ...
        <motion.div style={{ y: OnScrollmoveYup, opacity: logoOpacity }} className="w-full h-full flex items-center justify-center absolute" >
            <Image src={Logo} alt="..." className="w-[700px] md:w-[700px] opacity-100" />
        </motion.div>
        <motion.div style={{ y: OnScrollmoveYup, opacity: logoShadow }} className="w-full h-full flex items-center justify-center absolute" >
            <Image src={Logo} alt="..." className="w-[700px] md:w-[700px] opacity-100 blur-2xl absolute " />
        </motion.div>
        ...
        <div className="flex absolute w-full h-full p-3 md:p-5 justify-end items-end">
            ...
            <motion.div initial={{y: 4}} className="absolute" drag="x" style={{ x: dragX }} dragTransition={{ bounceStiffness: 600, bounceDamping: 100 }} whileHover={{ scale: 1.1 }} dragConstraints={{ top: 3, left: 0, right:maxDrag, bottom: 3 }} >
