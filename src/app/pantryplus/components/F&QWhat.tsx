      ...
      {FQA.map((item, index) => (
        <div key={index}>
          <motion.div className="flex flex-row justify-between py-3 items-center">
            <p className="text-lg sm:text-xl font-light "><WordPopping leading="mr-[5px] mb-0" phrase={item[0]}/></p>
              <motion.button className="px-[14px] pb-0 text-2xl rounded-full text-center  w-10 h-10 bg-black text-white"
              initial={{ rotate: 0 }}
              animate={{
          ...
          >
            <p className="text-medium sm:text-lg font-light px-5"><WordPopping leading=" mr-1 mb-0" phrase={item[1]} /></p>
          </motion.div>
        </div>
      ))}
    </motion.div>
