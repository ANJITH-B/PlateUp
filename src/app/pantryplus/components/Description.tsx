    ...
    <motion.div variants={container} whileInView='visible' initial='initial' className='flex'>
      <div className="text-[25px] font-semibold w-1/2 text-[#FBFEE1]">
        <div className="flex flex-col gap-5 md:gap-4 w-full items-start">
          <div className={`${questrial.className} text-xl md:text-4xl mr-5`}><WordPopping leading="md:mb-2 mr-2 lg:mb-3" phrase="Experience the freedom of Grocery shopping without the hassle of payment" /></div>
          <div className="md:hidden block"><AppLink/></div>
        </div>
      </div>
      <div className="flex flex-col items-start gap-5 md:gap-4 w-1/2">
        <motion.p variants={container} whileInView='visible' initial='initial' className={`${outfit.className}  pl-4 md:pl-0 text-medium sm:text-lg md:text-2xl text-[#fbfee1d8]`}>
...
const AppLink: React.FC = () => {
  return (
    <main className='gap-5 md:gap-4 flex flex-col'>
      <div className={`${outfit.className} text-sm sm:text-medium md:text-xl font-light`}>
