  ...
  return (
    <div className="w-full flex justify-center items-center ">
      <Navbar
        ...
        isBordered
        className="max-w-screen-xl  bg-black border border-orange-200 rounded-full"
      >
        ...
        <NavbarContent
          className="hidden sm:flex gap-1 text-md text-white "
          justify="end"
        >
          {menuItems.map((item, index) => {
            const className = `w-6 h-1 text-black ${
              index <= path ? "bg-orange-300" : "bg-black"
            }`;
            if (index)
              return (
                <>
                  ...
                  <NavbarItem>
                    <TransitionLink href={item.href} label={item.name} />
                  </NavbarItem>
                </>
              );
          })}
        ...
        <NavbarContent className="w-full">
          ...
          <button className="bg-orange-600 rounded-xl text-xs w-40 h-8 hidden sm:block py-2 px-4">
            Get<span className="text-orange-600">_</span>Started
          </button>
        ...
        <NavbarMenu>
          {menuItems.map((item, index) => (
            <NavbarMenuItem key={`${item}-${index}`}>
              <TransitionLink
                className="w-full mx-auto text-center font-bold text-xl py-2   hover:text-orange-500"
                href={item.href}
                size="lg"
                label={item.name}
              />
